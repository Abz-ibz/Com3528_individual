...
class FaceRecognizerNode:
    def __init__(self):
        rospy.init_node('face_id_right_node')
        ...
        self.publisher = rospy.Publisher(TOPIC_FACE_ID_RIGHT, FaceData, queue_size=10)
        ...
        rospy.Subscriber(CAMERA_SOURCE_RIGHT, CompressedImage, self.callback)
        log_info("[BOOT - RIGHT] Node initialized and camera subscribed.")
    ...
    def load_all_known_faces(self):
        ...
        for folder in RECOGNITION_FOLDERS:
            ...
            if not os.path.exists(full_path):
                log_warning(f"[LOAD FACES - RIGHT] Folder not found: {full_path}")
                continue
            for file in os.listdir(full_path):
                if file.lower().endswith((".jpg", ".png", ".jpeg")):
                    ...
                    if img is None:
                        log_warning(f"[LOAD FACES - RIGHT] Failed to load: {img_path}")
                        continue
                    rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                    dets = self.detector(rgb)
                    if not dets:
                        log_warning(f"[LOAD FACES - RIGHT] No face in image: {img_path}")
                        continue
    ...
    def callback(self, msg):
        try:
            ...
            if not dets:
                log_info("[CALLBACK - RIGHT] No faces detected in frame.")
                return
            ...
            for det in dets:
                ...
                log_info(f"[PUBLISH - RIGHT] Sent FaceData for: {msg_out.name} (Conf: {msg_out.confidence})")
                ...
                log_face_recognition(f"[RIGHT] {name} ({folder}) - Conf: {round(1 - min_dist, 3)}")

        except Exception as e:
            log_error(f"[CALLBACK ERROR - RIGHT] {e}")
            traceback.print_exc()

if __name__ == '__main__':
    try:
        log_info("[BOOT - RIGHT] Starting face recognizer node...")
        FaceRecognizerNode()
        rospy.spin()
    except Exception as e:
        log_error(f"[FATAL INIT ERROR - RIGHT] {e}")
        traceback.print_exc()
